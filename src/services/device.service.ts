...
export const saveDevice = async (fingerprint: string, name?: string, type?: string, os?: string, app?: string, lastSessionId?: string, lastSessionUserId?: string): Promise<IDevice> => {
...
export const getDeviceById = async (deviceId: string): Promise<IDevice> => {
    const device = await DeviceModel.findById(deviceId).lean();
    if(!device) throw DeviceError.NOT_FOUND;
    return device;
};

export const getDeviceByFingerprint = async (fingerprint: string): Promise<IDevice> => {
    const device = await DeviceModel.findOne({ fingerprint }).lean();
    if(!device) throw DeviceError.NOT_FOUND;
    return device;
};

export const getDevicesByIds = async (ids: string[]): Promise<IDevice[]> => {
    return await DeviceModel.find({ _id: { $in: ids } }).sort({ added_at: -1 });
};

export const updateDeviceLastSessionById = async (id: string, lastSessionId: string, lastSessionUserId: string): Promise<IDevice|null> => {
